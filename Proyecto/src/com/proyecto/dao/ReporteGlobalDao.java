...
public class ReporteGlobalDao {
		...
		public List<ReporteGlobal> ListaGlobal(){   
			...
			String sql = "select cli.rut, cli.nombre, vis.Fecha, vis.IDVisita, cap.Fecha, cap.NumAsistentes, "
					+ "ase.Fecha, ase.Gestion, pag.IDPago, pag.FechaPago, pag.MontoRegular, pag.MontoAdicional "
					+ "from cliente 'cli' inner join accidentes 'acc' on (cli.rut = acc.rut_id) inner join visitas"
					+ " 'vis' on ( cli.rut = vis.rut_id) inner join pagos 'pag' on (cli.rut =pag.rut_id) inner "
					+ "join capacitaciones 'cap' on (cap.visitas_idvisita = vis.IDvisita) inner join asesorias 'ase' on "
					+ "(ase.visitas_idvisita = vis.IDvisita)";
