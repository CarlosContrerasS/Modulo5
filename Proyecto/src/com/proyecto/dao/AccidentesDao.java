...
public class AccidentesDao {
	...
	public int AgregarAccidenteM(Accidentes acc){    
	    String sql="insert into Accidentes (IdAccidente, Fecha, Hora, Suceso, Lugar, Rut_Id, "
	    		+ "Dias_Perdidos, Num_Trab ) values   ( ' "+acc.getIdAccidente()+"', '"+ acc.getFecha() +"', '"+acc.getHora()+"','"+acc.getSuceso()+"', '"+acc.getLugar()+"', '"+acc.getRut_Id()+"', '"+acc.getDias_Perdidos()+"', '"+acc.getNum_Trab()+"')";
